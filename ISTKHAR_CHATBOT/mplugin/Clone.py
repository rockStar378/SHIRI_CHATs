...
from ISTKHAR_CHATBOT import CLONE_OWNERS
from ISTKHAR_CHATBOT import ISTKHAR_CHATBOT as app, save_clonebot_owner
from ISTKHAR_CHATBOT import db as mongodb, ISTKHAR_CHATBOT
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="ISTKHAR_CHATBOT/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
        ...
        try:
            ...
            await mi.edit_text(
                f"⚠️ <b>Error:</b>\n\n<code>{e}</code>\n\n**Forward this message to @THE_ISTKHAR_BOY_OP for assistance**"
            )
    else:
